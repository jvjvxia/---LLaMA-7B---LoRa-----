...
CPU：16核心
内存：120GB
存储：50GB数据盘（存放模型权重、数据集与训练日志）
训练时长：初始训练3轮约1.2小时，调参后完整10轮训练耗时约3.5小时，单轮训练约21分钟。

2. 软件环境
操作系统：Linux（Ubuntu 20.04）
编程语言：Python 3.8.10
核心依赖库：transformers==4.30.2、peft==0.4.0、torch==2.0.1、datasets==2.13.1、wandb==0.15.4（训练监控)
